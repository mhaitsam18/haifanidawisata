						...
						<tr>
							...
							<td>
								<?php if ($row['sbt'] == 'Belum dikonfirmasi'): ?>
									<a href="<?= base_url('Transaksi/updateStatusPembayaranPaket/').$row['idbt'].'/tidak valid' ?>" class="badge badge-danger" onclick="return confirm('Are you sure?');">Tidak Valid</a>
									<a href="<?= base_url('Transaksi/updateStatusPembayaranPaket/').$row['idbt'].'/valid' ?>" class="badge badge-success" onclick="return confirm('Are you sure?');">Valid</a>
								<?php else: ?>
